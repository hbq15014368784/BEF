...
class FSRU(nn.Module):
    def __init__(self, d_model, seq_len=14, dropout=0., mlp_ratio=4., num_filter=2, num_class=2, num_layer=1):
        super(FSRU, self).__init__()

        # Text
        s = seq_len // 2 + 1
        self.text_proj = nn.Linear(1, d_model) 
        self.text_norm = nn.LayerNorm(d_model)
        self.text_pos_embed = TextPositionEmbed(seq_len, d_model, dropout)

        # Image
        n = 36 // 2 + 1 
        self.img_proj = nn.Linear(2048, d_model)
        self.img_norm = nn.LayerNorm(d_model)
        self.img_pos_embed = nn.Parameter(torch.zeros(1, 36, d_model))
    ...
    def forward(self, text, image):
        ...
        text = text.unsqueeze(-1).float()  # [B, 14] -> [B, 14, 1]
        text = self.text_proj(text)  # [B, 14, d_model]
        text = self.text_norm(text)
        text = self.text_pos_embed(text)
