...
def miller_recurrence(nu, x):
    ...
    for i in range(2*nu, 0, -1):
        ...
        if torch.isinf(I_n_tem).any():
            I_n1_clone = I_n1.clone()
            I_n1_clone = I_n1_clone.expand_as(I_n)
            I_n1_clone /= I_n
            scale += torch.log(I_n)
            if i >= (nu+1):
            ...
            I_n, I_n1 = 2 * i / x * I_n + I_n1_clone, I_n.clone()
        else:
    ...
    ive0 = torch.tensor(i0e(x.cpu().numpy())).to(x.device)
...
class EstimatorCV():
    def __init__(self, feature_num, class_num):
        ...
        self.Ave = F.normalize(torch.randn(class_num, feature_num, dtype=torch.float64), dim=1) * 0.9
        self.Amount = torch.zeros(class_num, dtype=torch.float64)
        self.kappa = torch.ones(class_num, dtype=torch.float64) * self.feature_num * 90 / 19

        kappa_np = self.kappa.cpu().numpy()
        tem = torch.from_numpy(ive(self.feature_num / 2 - 1, kappa_np)).to(dtype=torch.float64)

        self.logc = (torch.log(tem + 1e-300) +
                     self.kappa -
                     (torch.tensor(self.feature_num / 2 - 1, dtype=torch.float64) *
                      torch.log(self.kappa + 1e-300)))
    ...
    def update_CV(self, features, labels):
        ...
        labels = labels.argmax(dim=1).long() if labels.dim() > 1 else labels.long()
